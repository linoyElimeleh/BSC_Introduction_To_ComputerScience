...
{
	//the function take array and remove all the duplicates number , return the numbers in the array after the removing
...
int CountSemiSimilar(char a[][M], char b[][M], int n)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		for (j = 0; j < 26; j++)
		{
			...
			{
				flag = 0;
				for (int k = 0; k < 26; k++) { // create this loop in order to remove all the value in the array
					arrayForCounterA[k] = 0;
					arrayForCounterB[k] = 0;
				}
			}
		}
		if (flag == 1)
